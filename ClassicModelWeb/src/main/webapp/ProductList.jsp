...
<div class="d-flex flex-row">
    ...
    <div class="px-1 mx-1 div-link" onclick="loadProduct(page=${page <= 1 ? totalPage : (page - 1)}, ${pageSize})">
        &lt;
    </div>
    <c:forEach begin="1" end="${requestScope.totalPage}" varStatus="vs">
        <c:choose>
            <c:when test="${vs.count == requestScope.page}">
                <div class="px-1 mx-1 div-link text-light bg-dark">
                        ${vs.count}
                </div>
            </c:when>
            <c:otherwise>
    ...
    <div class="px-1 mx-2 div-link" onclick="loadProduct(page=${page >= totalPage ? 1 : (page + 1)}, ${pageSize})">
        &gt;
    </div>
    <div class="px-4 mx-1">items per page:
